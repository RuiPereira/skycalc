...

{
	/* Returns zone time offset when standard time zone is stdz,
	   ...
	   jdb, and the last is jde.  The parameter 
		...
		use_dst = 1  (USA convention, with 1986 and 2007 updates)
...

{
	...
	if(obs_code[0] == 'k') {	
		...
		*lat=get_coord();
		...
		printf("  1 ... USA, with 1986 and 2007 changes.\n");
...

{
	...
	if(use_dst == 1) {
		printf("DAYLIGHT SAVINGS time is used using\nconventions ");
		printf("for the USA; for 2007+, 2nd Sunday in March to ");
		printf("first Sunday in November.");
	}
	else if (use_dst != 0) 
